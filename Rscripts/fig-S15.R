...
ggsave(
  "outputs/Visualization/fig-S15b.pdf",
  width = 12, height = 3.6
)

# CNTN1 & NOTCH1 co-localization (Fig. S15c) ####
...
pic.c +
  geom_point(
    data = plot.data,
    size = 1.2, stroke = 0, color = "white", shape = 21,
    mapping = aes(x = x, y = y, fill = l.value)
  ) +
  scale_fill_gradient2(
    low = "#8C9AA4", mid = "#FFFFFF", high = "#EE692E",
  ...
  scale_color_gradient2(
    low = "#8C9AA4", mid = "#FFFFFF", high = "#9A1DEE",
...
ggsave(
  "outputs/Visualization/fig-S15c.pdf",
  width = 10, height = 4
)

# PTN & PTPRS co-localization (Fig. S15d) ####
plot.data <- data.frame(
...
ggsave(
  "outputs/Visualization/fig-S15d.pdf",
  width = 10, height = 4
)
